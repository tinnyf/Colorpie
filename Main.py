...
bot = BotInit(intents=intents, application_id=974427338857132033,
              command_prefix=['~', '-', "Oh 5 Colors, hear me now ", "5 Colors, I bid thee ", "5 colors, I bid thee"])
...
async def report_message(interaction: discord.Interaction, message: discord.Message):
    ...
    await log_channel.send(
        f"Reported by {interaction.user.name}. {interaction.guild.get_role(696382214207832145).mention}", embed=embed,
        view=url_view)


@discord.app_commands.context_menu(name="See relics")  # Can't be defined in a class
async def see_relics(interaction, user: discord.Member):
    await interaction.response.send_message(
...
async def AnonSuggest(interaction: discord.Interaction):
    ...
    suggestion = discord.ui.TextInput(placeholder="We should all change our names to be themed", required=True,
                                      label="Suggestion body", style=discord.TextStyle.paragraph)
    ...
    async def callback(interaction: discord.Interaction):
        ...
        await interaction.response.send_message("Thanks for your suggestion! It's with staff right now!",
                                                ephemeral=True)
